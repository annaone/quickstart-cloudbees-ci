...
----
Agents are resource intensive, so we recommend separating them from Managed Controllers. The Partner Solution provides a method for doing so with the additional agent pools.
