...
image::../images/unlock-cbci.png[CloudBees CI Activation Screen]

1. Obtain the *_initialAdminPassword_* by accessing the EKS cluster from the bastion host. To log in to the bastion host, find the BastionIP in the Outputs tab of the base {partner-product-name} stack. Then connect from your local terminal to the bastion host by using SSH, with a command similar to the following:
